...
class VocalRemoverTrainingSet(torch.utils.data.Dataset):
    def __init__(
        self, training_set, cropsize, reduction_rate, reduction_weight, mixup_rate, mixup_alpha
    ):
...
def train_val_split(dataset_dir, split_mode, val_rate, val_filelist):
    if split_mode == "random":
        filelist = make_pair(
            os.path.join(dataset_dir, "mixtures"), os.path.join(dataset_dir, "instruments")
        )
...
if __name__ == "__main__":
    ...
    for mix_path, inst_path in tqdm(filelist):
        ...
        X_spec, y_spec, _, _ = spec_utils.cache_or_load(mix_path, inst_path, 44100, 1024, 2048)
        ...
        outpath = f"{outdir}/{mix_basename}_Vocal.jpg"
