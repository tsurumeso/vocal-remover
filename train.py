...
def main():
    ...
    training_set = dataset.make_training_set(
        filelist=train_filelist, sr=args.sr, hop_length=args.hop_length, n_fft=args.n_fft
    )
    ...
    val_dataset = dataset.VocalRemoverValidationSet(patch_list=patch_list)
    ...
    for epoch in range(args.epoch):
        ...
        train_loss = train_epoch(
            train_dataloader, model, device, optimizer, args.accumulation_steps
        )
        ...
        with open(f"loss_{timestamp}.json", "w", encoding="utf8") as f:
            json.dump(log, f, ensure_ascii=False)


if __name__ == "__main__":
    timestamp = datetime.now().astimezone().strftime("%Y%m%d%H%M%S")
    logger = setup_logger(__name__, f"train_{timestamp}.log")

    try:
        main()
    except Exception:
        logger.exception("An exception occurred during training.")
