...
def main():
    ...
    print("done")
    sf.write(f"{output_dir}{basename}_Instruments.wav", wave.T, sr)

    print("inverse stft of vocals...", end=" ")
    wave = spec_utils.spectrogram_to_wave(v_spec, hop_length=args.hop_length)
    print("done")
    sf.write(f"{output_dir}{basename}_Vocals.wav", wave.T, sr)

    if args.output_image:
        image = spec_utils.spectrogram_to_image(y_spec)
        utils.imwrite(f"{output_dir}{basename}_Instruments.jpg", image)

        image = spec_utils.spectrogram_to_image(v_spec)
        utils.imwrite(f"{output_dir}{basename}_Vocals.jpg", image)
